...
## Task
Implement Monte Carlo policy evaluation, Temporal Difference policy evaluation, and Q-Learning for Blackjack. 
In all of them, the discount factor is 0.95. When the player wins, give reward +1, and when loses, the reward is -1. (See **Blackjack Rules** below for details).
...
Implement the Q-learning algorithm. Use epsilon=0.4 in the epsilon-greedy strategy in your final submission, but you are encouraged to check the behavior difference for various choices of epsilon. After learning, AutoPlay will follow the Q-learning values to make decisions. 
...
## Grading
Only submit `ai.py` file onto Gradescope
- Full (5 points): Pass all convergence tests on Gradescope.
- Almost (3 points): Passes at least 90% of given tests.
- Nothing (0 point): No attempt or passes less than 90% of given tests.
...
2. If dealer has A’s in its hand, then the A only counts as 1 when otherwise the dealer busts. See below for examples
    1. If dealer has {“Ace”, 6}. then the ace counts as 11 and dealer stops taking cards since the sum equals 17.
    2. If dealer has {8, 6, “Ace”}, then the ace counts as 1 (since counting A as 11 would make the dealer bust), and dealer keeps taking cards since the sum equals 15.
